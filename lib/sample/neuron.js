...
class NeuronModelDefinition {
    ...
    sequelizeImport(sequelize, DataTypes) {
        ...
        Neuron.isDuplicate = (idString, injectionId, id = null) => __awaiter(this, void 0, void 0, function* () {
            if (!injectionId || !idString) {
                return false;
            }
            const injection = yield Neuron.InjectionModel.findById(injectionId);
            if (!injection) {
                return false;
            }
            ...
            const dupes = yield Neuron.findAll({ where: { injectionId: { $in: injectionIds }, idString } });
            return dupes.length > 0 && (!id || (id !== dupes[0].id));
        });
        Neuron.isDuplicateNeuronObj = (neuron) => __awaiter(this, void 0, void 0, function* () {
        ...
        Neuron.createFromInput = (neuron) => __awaiter(this, void 0, void 0, function* () {
            ...
            else if (!util_1.isNull(neuron.brainAreaId)) {
                // Zero-length string or undefined
                neuron.brainAreaId = null;
            }
            if (yield Neuron.isDuplicateNeuronObj(neuron)) {
        ...
        Neuron.updateFromInput = (neuron) => __awaiter(this, void 0, void 0, function* () {
            ...
            const isDupe = yield Neuron.isDuplicate(neuron.idString || row.idString, neuron.injectionId || row.injectionId, row.id);
            if (isDupe) {
                throw { message: `A neuron id "${neuron.idString}" already exists on this sample` };
            }
            // Undefined is ok (no update) - null, or empty is not - unless it is already that way from create
            if (modelUtil_1.isNullOrEmpty(neuron.idString) && row.idString) {
